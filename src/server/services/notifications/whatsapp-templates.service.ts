...
export async function sendRegistrationApproval(data: {
  ...
  try {
    ...
    const variables = {
      ...
      invoicePdfLink: data.invoiceNumber ? `${company?.baseUrl || 'http://localhost:3000'}/invoice/${data.invoiceNumber}` : '-',
...
export async function sendInvoiceReminder(data: {
  ...
  try {
    ...
    const variables = {
      ...
      invoiceWebLink: `${data.companyName ? company?.baseUrl : 'http://localhost:3000'}/invoice/${data.invoiceNumber}`,
      invoicePdfLink: `${data.companyName ? company?.baseUrl : 'http://localhost:3000'}/invoice/${data.invoiceNumber}`,
      bankAccounts: bankAccountsText,
