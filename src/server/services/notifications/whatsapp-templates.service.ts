...
export async function sendRegistrationApproval(data: {
  ...
  try {
    ...
    const variables = {
      ...
      invoiceWebLink: data.invoiceNumber ? `${company?.baseUrl || 'http://localhost:3000'}/invoice/${data.invoiceNumber}` : '-',
      invoicePdfLink: data.invoiceNumber ? `${company?.baseUrl || 'http://localhost:3000'}/invoice/${data.invoiceNumber}/pdf` : '-',
...
export async function sendInstallationInvoice(data: {
  ...
  try {
    ...
    const variables = {
      ...
      dueDate: dueDateStr,
      paymentLink: data.paymentLink,
      invoicePdfLink: `${company?.baseUrl || 'http://localhost:3000'}/invoice/${data.invoiceNumber}/pdf`,
...
export async function sendInvoiceReminder(data: {
  ...
  try {
    ...
    const variables = {
      ...
      paymentLink: data.paymentLink,
      invoiceWebLink: `${data.companyName ? company?.baseUrl : 'http://localhost:3000'}/invoice/${data.invoiceNumber}`,
      invoicePdfLink: `${data.companyName ? company?.baseUrl : 'http://localhost:3000'}/invoice/${data.invoiceNumber}/pdf`,
      bankAccounts: bankAccountsText,
