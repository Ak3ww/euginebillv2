...
# /radius add address=<RADIUS_VPN_IP> secret=${nasSecret} service=ppp,hotspot src-address=${vpnIp}`
    ...
    if (vpnType === 'wireguard') {
      nasSetupScript = `
# ============================================
# EugineBill WireGuard VPN Client + RADIUS Setup Script
# NAS: ${name}
...
/interface wireguard add listen-port=0 name=wg0-EugineBill comment="EugineBill WireGuard"

# --- STEP 2: Set Client Private Key ---
# (Private key generated by EugineBill - keep secret!)
/interface wireguard set wg0-EugineBill private-key="${clientPrivateKey}"

# --- STEP 3: Add Peer (VPN Server CHR) ---
/interface wireguard peers add interface=wg0-EugineBill public-key="${serverWgPublicKey}" endpoint-address=${vpnServer.host} endpoint-port=${wgPort} allowed-address=0.0.0.0/0 persistent-keepalive=25 comment="EugineBill VPN Server"

# --- STEP 4: Assign VPN IP ---
/ip address add address=${vpnIp}/24 interface=wg0-EugineBill comment="EugineBill VPN IP"
...
`.trim()
      ...
      nasSetupScript = `
# ============================================
# EugineBill ${vpnTypeUpper} VPN Client + RADIUS Setup Script
# NAS: ${name}
# VPN Server: ${vpnServer.host}
# VPN IP: ${vpnIp}
# Generated: ${new Date().toISOString()}
...
:if ([/interface ${interfaceType} get [find name="${interfaceType}-EugineBill"] running] = true) do={
  ...
  try {
    ...
    if (client.vpnType?.toUpperCase() !== 'WIREGUARD' && vpnServer) {
      try {
        ...
        const nats = await mtik.execute('/ip/firewall/nat/print', [`?comment=EugineBill-VPN-${client.username}`])
