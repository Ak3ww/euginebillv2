#!/bin/bash
# ============================================================================
# EugineBill RADIUS VPS Uninstaller - Complete Removal
# ============================================================================
...
# Try to detect APP_USER from directory ownership, fallback to EugineBill
if [ -d "$APP_DIR" ]; then
...
# Backup directory
BACKUP_DIR="/root/EugineBill-backup-$(date +%Y%m%d-%H%M%S)"
...
show_warning() {
    ...
    echo -e "${RED}║                                                                   ║${NC}"
    echo -e "${RED}║  This script will COMPLETELY REMOVE all EugineBill RADIUS components ║${NC}"
    ...
    echo "  ❌ PM2 processes (EugineBill-radius, EugineBill-cron)"
...
backup_configs() {
    ...
    if [ -f "/etc/nginx/sites-available/EugineBill-radius" ]; then
        cp /etc/nginx/sites-available/EugineBill-radius "$BACKUP_DIR/nginx-backup" 2>/dev/null || true
    fi
...
kill_port_processes() {
    ...
    # Ports used by EugineBill RADIUS
...
stop_all_services() {
    ...
    # Stop PM2 processes (both root and EugineBill user)
...
remove_nginx_config() {
    ...
    rm -f /etc/nginx/sites-available/EugineBill-radius
    rm -f /etc/nginx/sites-enabled/EugineBill-radius
...
clean_logs() {
    ...
    rm -rf /var/log/EugineBill-vps-install.log
    rm -rf /var/log/freeradius
    rm -rf /var/log/nginx/EugineBill-radius-*
...
main() {
    ...
    echo "  cd /root/EugineBill-RADIUS-main/vps-install"
