#!/bin/bash
# ============================================================================
# EugineBill RADIUS VPS Installer - MySQL Module
# ============================================================================
...
safe_restart_mysql_with_fallback() {
    ...
    print_warning "${MYSQL_SERVICE} failed after applying tuning config. Rolling back EugineBill-perf.cnf..."
    if [ -f /etc/mysql/mysql.conf.d/EugineBill-perf.cnf ]; then
        mv /etc/mysql/mysql.conf.d/EugineBill-perf.cnf "/etc/mysql/mysql.conf.d/EugineBill-perf.cnf.disabled.$(date +%s)" || true
    fi
...
create_database() {
    ...
    if [ "$DB_EXISTS" -eq "1" ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            local BACKUP_FILE="/root/EugineBill_radius_backup_$(date +%Y%m%d_%H%M%S).sql"
...
configure_mysql_timezone() {
    ...
    cat > /etc/mysql/mysql.conf.d/EugineBill-perf.cnf <<EOF
[mysqld]
# ============================================================
# EugineBill RADIUS - MySQL Tuning (auto-generated by installer)
# RAM: ${TOTAL_RAM_MB}MB
# ============================================================
